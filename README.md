...
- Panes: Command-D splits side by side; Command-Shift-D stacks panes; Command-Return toggles zoom for the focused pane. Drag the divider to resize. Drag a pane’s header to the top, bottom, left, or right edge of another pane in the same tab. A translucent rectangle previews its half of the new split. Dropping moves the pane there and collapses its old split; running shells are preserved. Restore the split layout before dragging a zoomed pane. Use the pane header to focus, zoom, rename, start an agent, or close.
