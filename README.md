...
- Links: click a web URL or labeled hyperlink in terminal output to open it in your default browser, including in mouse-enabled applications such as Claude Code. Command-click also works; dragging selects text and copies it on release. Forwarding other clicks to applications requires the server to include application mouse modes in its terminal stream; stock Herdr 0.9.0 does not provide them.
