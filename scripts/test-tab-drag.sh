...
set -euo pipefail
cd "$(dirname "$0")/.."
TEST_BUILD="$PWD/.build/tab-drag-tests"
mkdir -p "$TEST_BUILD"
...
swiftc -parse-as-library -I "$TEST_BUILD" -L "$TEST_BUILD" -lHerdrCore \
    ...
    Sources/HerdrMac/SessionStore.swift Tests/HerdrMacTests/TabDragTests.swift \
    -o "$TEST_BUILD/TabDragTests"
"$TEST_BUILD/TabDragTests"
