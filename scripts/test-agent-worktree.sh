...
mkdir -p "$TEST_BUILD"
swiftc -emit-library -emit-module -module-name HerdrCore Sources/HerdrCore/*.swift Sources/HerdrCore/Appearance/*.swift \
...
swiftc -parse-as-library -I "$TEST_BUILD" -L "$TEST_BUILD" -lHerdrCore \
    ...
    Sources/HerdrMac/Appearance/AppearanceStore.swift Sources/HerdrMac/SessionStore.swift Tests/HerdrMacTests/AgentWorktreeTests.swift \
