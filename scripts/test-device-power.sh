...
set -euo pipefail
cd "$(dirname "$0")/.."
TEST_BUILD="$PWD/.build/device-power-tests"
mkdir -p "$TEST_BUILD"
...
swiftc -parse-as-library -I "$TEST_BUILD" -L "$TEST_BUILD" -lHerdrCore \
    ...
    Sources/HerdrMac/SessionStore.swift Tests/HerdrMacTests/DevicePowerPollingTests.swift \
    -o "$TEST_BUILD/DevicePowerPollingTests"
"$TEST_BUILD/DevicePowerPollingTests"
